...
group "all" {
  targets = ["base", "sdxl", "sd3", "flux1-schnell", "flux1-dev", "flux1-dev-fp8", "z-image-turbo", "base-cuda12-8-1", "illustrious", "qwen-image", "qwen-image-edit"]
}
...
target "qwen-image" {
  ...
  tags = ["${DOCKERHUB_REPO}/${DOCKERHUB_IMG}:${RELEASE_VERSION}-qwen-image", "${DOCKERHUB_REPO}/${DOCKERHUB_IMG}:latest"]
  inherits = ["base"]
}
...
target "qwen-image-edit" {
  ...
  tags = ["${DOCKERHUB_REPO}/${DOCKERHUB_IMG}:${RELEASE_VERSION}-qwen-image-edit", "${DOCKERHUB_REPO}/${DOCKERHUB_IMG}:latest"]
  inherits = ["base"]
}
